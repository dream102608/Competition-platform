...
// V1.5：赛项筛选全联动 + 评审绩效下钻明细 + 状态×赛项双维导出
// 纯前端实时聚合 + 纸墨风 CSS 图表；仅审批角色（教师/系级/院级/校级）可看
const app = getApp();
...
Page({
  ...
  refresh() {
    ...
    const comp = this.data.compFilter || 'all';
    const sAll = data.reportStats();          // 全量 → 赛项 chips 计数（不随筛选变化）
    const s = data.reportStats(comp);          // 按赛项过滤后的展示聚合
    ...
    // V1.4：评审绩效——按审批人已办量归一，条内 pass/reject 分色
    ...
    const compFilters = sAll.byComp.map(x => ({ k: x.k, v: x.v, short: short(x.k, 10) }));
    const filterLabel = FILTER_LABEL[this.data.filter] || '全部报名';
    const exportHint = comp === 'all' ? filterLabel : filterLabel + ' · ' + comp;
    this.setData({
      guard: false,
      ...
      filters: [{ key: 'all', label: '全部', v: total }]
        .concat(STATUS_META.map(m => ({
          key: m.key, label: m.label, v: statusByKey[m.key].v
        }))),
      compFilters, compAllTotal: sAll.total,
      compName: comp === 'all' ? '' : comp,
      exportHint
    });
  },
  ...
  /* V1.5：赛项筛选（联动全部图表 + 导出） */
  setCompFilter(e) {
    const c = e.currentTarget.dataset.comp;
    this.setData({ compFilter: c });
    this.refresh();
  },
  clearComp() {
    this.setData({ compFilter: 'all' });
    this.refresh();
  },

  /* V1.5：评审绩效 → 下钻该审批人逐单明细 */
  openDrill(e) {
    const name = e.currentTarget.dataset.name;
    const rows = data.approverDetail(name);
    this.setData({ drill: { name, rows }, drillShow: true });
  },
  closeDrill() { this.setData({ drillShow: false }); },
  noop() {},

  back() { wx.navigateBack({ fail: () => wx.switchTab({ url: '/pages/home/home' }) }); },
  goHome() { wx.switchTab({ url: '/pages/home/home' }); },

  /* 按当前 状态×赛项 双维筛选导出 CSV（V1.5 修复：此前 exportCSV 忽略状态参数） */
  exportCSV() {
    const f = this.data.filter;
    const c = this.data.compFilter;
    const csv = data.exportCSV(f, c);
